...
router.post('/add', 
    checkBUName,
    async (ctx, next) => {
        const { desc } = (ctx.request.body as any);
    ...
    async (ctx) => {
        const { BUName, desc } = (ctx.request.body as any);
...
router.post('/pick/component', 
    ...
    checkComponentParam,
    async (ctx) => {
        const { packageName, BUName, version, description, title, componentName, iconUrl } = (ctx.request.body as any)
